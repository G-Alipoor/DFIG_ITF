function [modes its]=ceemd(x,Nstd,NR,MaxIter,max_modes)
%--------------------------------------------------------------------------
...
[modes, o, it]=emd(xconruido,'MAXITERATIONS',MaxIter,'MAXMODES',max_modes-1);
modes=modes/NR;
iter=it;
if NR>=2
    for i=2:NR
        xconruido=x+Nstd*randn(size(x));
        [temp, ort, it]=emd(xconruido,'MAXITERATIONS',MaxIter,'MAXMODES',max_modes-1);
        temp=temp/NR;
        lit=length(it);
        [p liter]=size(iter);
        if lit<liter
        ...
        [alto ancho]=size(modes);
        diferencia=alto-filas;
        if filas>alto
            modes=[modes; zeros(abs(diferencia),ancho)];
        end;
        if alto>filas
